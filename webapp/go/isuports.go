...
func competitionRankingHandler(c echo.Context) error {
	...
	key := "get_ranking:" + competitionID
	ranking, err, _ := group.Do(key, func() (interface{}, error) {
		now := time.Now().Unix()
		var tenant TenantRow
		if err := adminDB.GetContext(ctx, &tenant, "SELECT * FROM tenant WHERE id = ?", v.tenantID); err != nil {
			return nil, fmt.Errorf("error Select tenant: id=%d, %w", v.tenantID, err)
		}
		...
		if rankAfterStr != "" {
			if rankAfter, err = strconv.ParseInt(rankAfterStr, 10, 64); err != nil {
				return nil, fmt.Errorf("error strconv.ParseUint: rankAfterStr=%s, %w", rankAfterStr, err)
			}
		}
		...
		for _, ps := range pss {
			...
			p, err := retrievePlayer(ctx, ps.PlayerID)
			if err != nil {
